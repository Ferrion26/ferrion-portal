...
const {
  Document, Packer, Paragraph, TextRun, ImageRun, Table, TableRow, TableCell, Footer, PageBreak,
...
function quoteBody(cfg) {
  return [
    // ── TEIL A — Verkaufsdokument (Seiten 1–2) ──────────────
    headerBand(cfg), goldRule(), issuerMeta(cfg), ...billTo(cfg),

    sectionH("1)  IHRE AUSGANGSLAGE & ZIELSETZUNG"),
    para("Ausgangslage: {customer_situation}"),
    para("Zielsetzung: {customer_goal}"),

    sectionH("2)  UNSER LÖSUNGSVORSCHLAG"),
    ...
    sectionH("3)  WARUM FERRION"),
    bullet("Datenbank-Expertise, die im klassischen Systemhaus selten ist — wir denken in Workloads, nicht in Boxen"),
    bullet("Zertifizierter Partner von Huawei, Pure Storage und Commvault; NVIDIA-Infrastruktur für AI-Workloads"),
    bullet("Bewährtes Migrations-Playbook — zuletzt 500 TB mit null Minuten ungeplanter Downtime"),
    bullet("Referenzprojekt: {reference_project}"),
    para("Ihr persönlicher Ansprechpartner für dieses Angebot: {contact_person}, {contact_phone}, {contact_email}."),

    sectionH("4)  ANGEBOTSDATEN & PREISE"),
    ...
    para("Der Auftraggeber erteilt hiermit den Auftrag zu den vorstehenden Leistungen und Preisen. Ergänzend gelten die Bestimmungen der Anlage 1 (Allgemeine Bestimmungen & Auftragsverarbeitungsvereinbarung) als vereinbart."),
    para("Bei Beauftragung bis {valid_until} ist ein Umsetzungsstart ab {start_date} möglich."),
    electronicNote(),
    signatures(),

    // ── TEIL B — Anlage 1: Rechtstexte ──────────────────────
    new Paragraph({ children: [new PageBreak()] }),
    p(run("ANLAGE 1", { size: 15, bold: true, color: GOLD, characterSpacing: 40 }), { spacing: { before: 80, after: 20 } }),
    p(run("ALLGEMEINE BESTIMMUNGEN & AUFTRAGSVERARBEITUNGSVEREINBARUNG", { size: 22, bold: true, color: INK, characterSpacing: 10 }), {
      spacing: { after: 60 },
      border: { bottom: { style: BorderStyle.SINGLE, size: 8, color: GOLD, space: 6 } },
    }),
    para("Diese Anlage ist integraler Bestandteil des Angebots {quote_number} und gilt mit der Auftragserteilung als vereinbart."),

    sectionH("A-1)  ALLGEMEINE BESTIMMUNGEN"),
    subH("A-1.1  Rücktrittsrecht"),
    ...
    subH("A-1.2  Gewährleistung, Wartung, Änderungen"),
    ...
    subH("A-1.4  Urheberrecht und Nutzung"),
    ...
    subH("A-1.6  Datenschutz und Geheimhaltung"),
    para("Die Vertragspartner behandeln alle im Rahmen der Zusammenarbeit erlangten, nicht offenkundigen Informationen vertraulich. Die Verarbeitung personenbezogener Daten erfolgt nach der DSGVO; Näheres regelt die Auftragsverarbeitungsvereinbarung in Punkt A-2."),

    sectionH("A-2)  AUFTRAGSVERARBEITUNGSVEREINBARUNG"),
    ...
    subH("A-2.4  Sub-Auftragsverarbeitung"),
    ...
    subH("A-2.6  Rückgabe von personenbezogenen Daten"),
    ...
    sectionH("A-3)  ZAHLUNGSBEDINGUNGEN & VERGÜTUNG"),
    subH("A-3.1  Vergütung"),
    para("Die Vergütung richtet sich nach den in Punkt 4 des Angebots angeführten Positionen. Alle Preise verstehen sich netto in {currency} zuzüglich der gesetzlichen Umsatzsteuer. Reise-, Fahrt- und Nächtigungskosten werden, sofern nicht anders vereinbart, nach tatsächlichem Aufwand verrechnet."),
    subH("A-3.2  Zahlungsbedingungen"),
    ...
    sectionH("A-5)  SCHLUSSBESTIMMUNGEN"),
