...
export const NETAPP_METRICS: MetricDefinition[] = [
  ...
  {
    key: "network_ports_down",
    label: { de: "Offline Netzwerk-Ports", en: "Offline Network Ports" },
    format: "count",
    aggregation: "last",
    section: "hardware",
    trendGood: "down",
    methodology: {
      de: "Anzahl der aktivierten Ethernet-/FC-Ports mit state ungleich \"up\" (Quelle: GET /api/network/ethernet/ports, GET /api/network/fc/ports) — Endpunkte/Feldnamen nach allgemeiner ONTAP-REST-Konvention, nicht gegen ein reales Gerät verifiziert.",
      en: "Number of enabled Ethernet/FC ports with state other than \"up\" (source: GET /api/network/ethernet/ports, GET /api/network/fc/ports) — endpoints/fields follow general ONTAP REST convention, not verified against a real device.",
    },
  },
  {
    key: "snapmirror_relationships_unhealthy",
    label: { de: "SnapMirror-Beziehungen mit Fehlstatus", en: "Unhealthy SnapMirror Relationships" },
    format: "count",
    aggregation: "last",
    section: "hardware",
    trendGood: "down",
    methodology: {
      de: "Anzahl der SnapMirror-Beziehungen mit healthy ungleich true (Quelle: GET /api/snapmirror/relationships) — Endpunkt/Feldnamen nach allgemeiner ONTAP-REST-Konvention, nicht gegen ein reales Gerät verifiziert.",
      en: "Number of SnapMirror relationships with healthy other than true (source: GET /api/snapmirror/relationships) — endpoint/fields follow general ONTAP REST convention, not verified against a real device.",
    },
  },
  {
    key: "aggregate_oversubscription_pct",
    ...
    derived: true,
    methodology: {
      de: "Je Aggregat: zugesagte (logische) Kapazität der darauf liegenden Volumes im Verhältnis zur physischen Aggregat-Kapazität (space.block_storage.size), als Maximum über alle Aggregate gemeldet (Worst-Case). Nur Volumes mit genau einem Aggregat werden gezählt. Rein informativ — kein geräteseitig konfigurierter Schwellwert bekannt.",
      en: "Per aggregate: committed (logical) capacity of the volumes on it relative to the aggregate's physical capacity (space.block_storage.size), reported as the maximum across all aggregates (worst case). Only volumes with exactly one aggregate are counted. Purely informational — no device-configured threshold known.",
    },
  },
  {
    key: "volumes_snapshot_reserve_exhausted",
    label: { de: "Volumes mit voller Snapshot-Reserve", en: "Volumes With Exhausted Snapshot Reserve" },
    format: "count",
    aggregation: "last",
    section: "hardware",
    trendGood: "down",
    severeIfNonZero: true,
    methodology: {
      de: "Anzahl der Volumes, bei denen space.snapshot.used die reservierte Snapshot-Kapazität (space.snapshot.reserve_percent × Volume-Größe) erreicht oder überschreitet (Quelle: GET /api/storage/volumes) — Felder nur durch Analogie zum space.*-Schema plausibel, nicht gegen ein reales Gerät verifiziert.",
      en: "Number of volumes where space.snapshot.used reaches or exceeds the reserved snapshot capacity (space.snapshot.reserve_percent × volume size) (source: GET /api/storage/volumes) — fields plausible only by analogy to the existing space.* schema, not verified against a real device.",
    },
  },
  {
    key: "volumes_unencrypted",
    label: { de: "Unverschlüsselte Volumes", en: "Unencrypted Volumes" },
    format: "count",
    aggregation: "last",
    section: "security",
    methodology: {
      de: "Anzahl der Volumes mit encryption.enabled = false (Quelle: GET /api/storage/volumes) — rein informativ für Compliance-Zwecke, Feld nur durch Analogie zum bereits genutzten ha.enabled-Muster plausibel, nicht gegen ein reales Gerät verifiziert.",
      en: "Number of volumes with encryption.enabled = false (source: GET /api/storage/volumes) — purely informational for compliance purposes, field plausible only by analogy to the existing ha.enabled pattern, not verified against a real device.",
    },
  },
  {
